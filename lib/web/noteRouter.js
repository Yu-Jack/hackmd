...
noteRouter.post('/remark/edit-note', jsonParser, wrap(async (req, res) => {
  const note_id = req.body.id;
  const note_new_name = req.body.name;
  const result = await models.Note.update({
    name: note_new_name
  }, {
    where: {
      id: note_id
...
noteRouter.post('/new-in-board', jsonParser, wrap(async (req, res) => {
  ...
  // link note
  const response = await axios.post(`${config.remarkEndpoint}/link-note`, {
